...
pub fn generate_open(rng: &mut rand::StdRng) -> map::Map {
    ...
    let width: usize = rng.gen_range(20, 60);
    let height: usize = rng.gen_range(20, 40);

    let open = map::Square {
        symbol: ' ',
        back_color: map::Color::LightGrey,
        fore_color: map::Color::Black,
    };
...
fn add_lake(map: &mut map::Map, rng: &mut rand::StdRng) {
    ...
    for v in 0..diameter {
        ...
        if y > 0.0 && y + 1.0 < map.height as f64 {
            let dy = y - center_y;
            for h in 0..diameter {
                let x = center_x - diameter as f64 / 2.0 + h as f64;
                if x > 0.0 && x + 1.0 < map.width as f64 {
                    let dx = x - center_x;
